...
def add_rate_of_change(
    ...
    cols = {
        f'{column.lower()}_pct_change_{p}d': (
            df.groupby('entity_id', sort=False)[column]
            .pct_change(periods=p)
            # pct_change divides by the value `p` periods earlier, which is
            # legitimately 0 on some rows (e.g. a no-burn/no-supply day) --
            # that produces +/-inf rather than NaN, and XGBoost rejects inf
            # outright. Treat it the same as any other undefined ratio.
