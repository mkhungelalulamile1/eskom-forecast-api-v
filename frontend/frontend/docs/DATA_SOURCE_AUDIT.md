...
**Canonical code:** `frontend/frontend/` (both committed production builds in `build/` compile from this tree — verified via the sourcemaps). The copy at `frontend/` (outer) is stale: its `src/` has no Python backend modules, so `frontend/main.py` cannot boot from it. The annotated shared data-flow files (services, hooks, context, `main.py`) have been mirrored to the outer copy; component annotations live in the canonical tree only.
...
**One nuance that runs through everything:** every endpoint is *dynamic*, but the backend falls back to **local parquet files** when Azure Storage env vars are absent (`src/ui.py read_parquet`). In dev those parquets are generated from `generate_mock_data.py` (synthetic Bronze) — so locally you can be looking at *mock data flowing through the real pipeline*. The values in `frontend/frontend/data/gold/*` in this repo are exactly that (e.g. daily `Input ≈ 12.7` for Arnot). With `AZURE_STORAGE_ACCOUNT_URL` + real SQL ingest configured, the same endpoints serve production data.
...
```
React SPA (frontend/frontend/src)
...
FastAPI (frontend/frontend/main.py)
...
| **F2** | Backend cannot boot from a fresh clone: `main.py` imports `training` (`deploy_models_to_azure`, `train_models`), `weather` and `scenario_definitions` from a `training/` folder **that is not committed** to this repo. | Anyone cloning the repo gets ImportError. The `__pycache__` folders prove it only runs where `training/` existed locally. |
...
2. **Commit the `training/` package** (`training.py`, `weather.py`, `scenario_definitions.py`, `additional_features.py`) or vendor the three imports — the service currently cannot start from the repo.
...
| `RUNBOOK.md`/pipelines: the frontend pipeline builds `frontend/Dockerfile`, which ships only the self-contained `index.html`. | The React SPA in `frontend/frontend/src` is **not built or deployed by any pipeline yet** — it is the in-development replacement. | ✔ matches (and explains the two-dashboard situation) |
